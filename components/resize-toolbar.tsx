...
export function ResizeToolbar({ selectedModel, onSizeChange, onResetSize }: ResizeToolbarProps) {
  ...
  useEffect(() => {
    if (selectedModel) {
      ...
      const avgScale = (currentSize[0] / originalSize[0] + currentSize[1] / originalSize[1] + currentSize[2] / originalSize[2]) / 3
  ...
  if (!selectedModel) {
    return (
      <Card className="bg-black/40 border-gray-800/50 backdrop-blur-sm">
        ...
        <CardContent>
          <p className="text-gray-400 text-sm">Select an object to resize</p>
        </CardContent>
      </Card>
    )
  }
    ...
    <Card className="bg-black/40 border-gray-800/50 backdrop-blur-sm">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="bg-gray-900/50 rounded-lg p-3">
          <p className="text-white text-sm font-medium">{selectedModel.name}</p>
          <p className="text-gray-400 text-xs">
            Current Size: {selectedModel.size?.[0]?.toFixed(2) || '1.00'} × {selectedModel.size?.[1]?.toFixed(2) || '1.00'} × {selectedModel.size?.[2]?.toFixed(2) || '1.00'}
          </p>
        </div>
        ...
        <div className="space-y-3">
          <Label className="text-gray-300 text-sm">Individual Axes</Label>
          
          <div className="space-y-2">
            <Label className="text-red-400 text-xs">X Axis (Width)</Label>
            <div className="flex gap-2">
              <Slider
                value={[xScale]}
                onValueChange={(value) => handleAxisScale('x', value[0])}
                min={0.1}
                max={10}
                step={0.1}
                className="flex-1"
              ...
              <Input
                ...
                onChange={(e) => handleAxisScale('x', parseFloat(e.target.value) || 1)}
          ...
          <div className="space-y-2">
            <Label className="text-green-400 text-xs">Y Axis (Height)</Label>
            <div className="flex gap-2">
              <Slider
                value={[yScale]}
                onValueChange={(value) => handleAxisScale('y', value[0])}
                min={0.1}
                max={10}
                step={0.1}
                className="flex-1"
              ...
              <Input
                ...
                onChange={(e) => handleAxisScale('y', parseFloat(e.target.value) || 1)}
                className="w-16 h-8 text-xs bg-gray-900/50 border-gray-700 text-white"
                step={0.1}
                min={0.1}
              />
          ...
          <div className="space-y-2">
            <Label className="text-blue-400 text-xs">Z Axis (Depth)</Label>
            <div className="flex gap-2">
              <Slider
                value={[zScale]}
                onValueChange={(value) => handleAxisScale('z', value[0])}
                min={0.1}
                max={10}
                step={0.1}
                className="flex-1"
              ...
              <Input
                ...
                onChange={(e) => handleAxisScale('z', parseFloat(e.target.value) || 1)}
                className="w-16 h-8 text-xs bg-gray-900/50 border-gray-700 text-white"
                step={0.1}
                min={0.1}
              />
        ...
        {/* Quick Actions */}
        <div className="space-y-2">
          <Label className="text-gray-300 text-sm">Quick Actions</Label>

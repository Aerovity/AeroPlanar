      ...
      <AlertDialogContent className="bg-gray-900 border-gray-700 text-white max-w-md">
        <AlertDialogHeader>
          <AlertDialogTitle className="flex items-center gap-2 text-white">
            <dialogIcon className={`w-5 h-5 ${dialogIconColor}`} />
            {dialogTitle}
          </AlertDialogTitle>
              ...
              {currentPerformance && (
                <div className="bg-black/30 p-3 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium text-gray-200">Current Scene Status</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs text-gray-300">
                    <div>Models: {currentPerformance.totalModels}</div>
                    <div>Score: {currentPerformance.performanceScore}/100</div>
                    <div>Faces: {currentPerformance.totalFaces.toLocaleString()}</div>
                  </div>
                </div>
              )}
              ...
              {isCritical && (
                <div className="bg-red-900/30 border border-red-600/50 rounded-lg p-3">
                  <div className="text-red-200 text-sm">
                    ...
                    <div>
                      Your browser may become unresponsive if you proceed. 
                      Please reduce scene complexity first.
                    </div>
                  </div>
                </div>
              )}
        ...
        <AlertDialogFooter className="flex-col space-y-2">
          ...
          {isCritical && (
            <div className="w-full space-y-2">
              <div className="text-xs text-gray-400 text-center">Quick Actions:</div>

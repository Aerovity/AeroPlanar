	...
	return (
		<Card className="[#080c0c]/15 border-gray-800/50 backdrop-blur-sm">
			...
			<CardContent className="space-y-4">
					...
					<div className="space-y-1">
						<div className="flex justify-between">
							...
							<span className="text-white">
								{performance.totalFaces.toLocaleString()}
							</span>
						</div>
						<div className="flex justify-between">
							<span className="text-gray-400">Models:</span>
							<span className="text-white">
								{performance.totalModels}
							</span>
						</div>
						<div className="flex justify-between">
							<span className="text-gray-400">Memory Est:</span>
							<span className="text-white">
								~{performance.estimatedMemoryUsage}MB
							</span>
						</div>
					</div>
					<div className="space-y-1">
						<div className="flex justify-between">
							...
							<span className="text-white">
								{performance.averageFacesPerModel.toLocaleString()}
							</span>
						</div>
						<div className="flex justify-between">
							<span className="text-gray-400">
							...
							<span className="text-white">
								{performance.largestModelFaces.toLocaleString()}
							</span>
						</div>
						<div className="flex justify-between">
				...
				{/* Action Buttons */}
				{(performance.warningLevel === "high" ||
					performance.warningLevel === "critical") && (
				...
				{performance.performanceScore < 60 && (
					<div className="mt-3 p-2 [#080c0c]/15 rounded-lg">
						...
						<div className="text-xs text-gray-300 space-y-1">
							...
							<div>
								• Consider using primitives instead of complex
								meshes
							</div>
							{performance.estimatedMemoryUsage > 50 && (
								<div>
									• High memory usage detected - restart
									browser if sluggish
								</div>
							)}
						</div>

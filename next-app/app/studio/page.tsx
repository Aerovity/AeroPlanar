...
import { Model3D } from "@/components/studio/model-3d";
import { Primitive3D } from "@/components/studio/primitive-3d";
import {
	GenerationPanel,
	type GenerationOptions,
} from "@/components/studio/generation-panel";
import { ArchitecturePanel } from "@/components/studio/architecture-panel";
import { Editing3DSidebar } from "@/components/studio/editing-3d-sidebar";
import { ResizeToolbar } from "@/components/studio/resize-toolbar";
import { ModelStats } from "@/components/studio/model-stats";
import { ScenePerformanceMonitor } from "@/components/studio/scene-performance-monitor";
import { PerformanceLimitDialog } from "@/components/studio/performance-limit-dialog";
...
import {
	useScenePerformance,
	estimateModelImpact,
} from "@/hooks/use-scene-performance";
...
import { ShareMockupDialog } from "@/components/studio/share-mockup-dialog";
import { MockupsGallery } from "@/components/studio/mockups-gallery";
...
export default function Home() {
	...
	const [currentView, setCurrentView] = useState<
		"generation" | "architecture" | "3d-editing" | "mockups"
	>("generation");
	...
	const [currentLightingFilter, setCurrentLightingFilter] =
		useState<string>("natural");
	...
	const [modifiedMeshes, setModifiedMeshes] = useState<
				...
				if (e.key === "c" && selectedModelId) {
					...
					if (modelToCopy) {
						...
						toast({
							title: "Model Copied",
							description:
								"Model copied to clipboard. Press Ctrl+V to paste.",
						});
					}
					...
					if (modelToCut) {
						...
						toast({
							title: "Model Cut",
							description:
								"Model cut to clipboard. Press Ctrl+V to paste.",
						});
					}
				}
			}
	...
	const handleModelPositionChange = (
		modelId: string,
	...
	const handleModelSizeChange = (
		...
		setModels((prev) =>
			prev.map((model) =>
				model.id === modelId ? { ...model, size: clampedSize } : model
			)
		);
	};
	...
	const handleResetSize = (modelId: string) => {
		...
		if (model && model.originalSize) {
			setModels((prev) =>
				prev.map((m) =>
					m.id === modelId ? { ...m, size: model.originalSize } : m
				)
			);
		}
	};
	...
	const handleMeshModification = useCallback(
		(modelId: string, mesh: THREE.Mesh | null) => {
	...
	const handleUpload3DModel = useCallback(
		async (file: File) => {
			try {
				...
				const estimatedFaces = Math.min(
					200000,
					Math.max(5000, Math.floor(fileSizeMB * 10000))
				);
	...
	const handleGenerate = useCallback(
		async (file: File, options: GenerationOptions) => {
			...
			try {
				...
				formData.append(
					"texture_resolution",
					options.textureResolution.toString()
				);
	...
	const handleGenerateFromText = useCallback(
		async (prompt: string, options: GenerationOptions) => {
			...
			try {
				...
				const response = await fetch(
					"http://localhost:8000/convert/text-to-3d",
					{
						method: "POST",
						body: formData,
					}
	...
	const handleGenerateMultimodal = useCallback(
		async (file: File, prompt: string, options: GenerationOptions) => {
			setIsGenerating(true);
			...
			try {
				...
				const response = await fetch(
					"http://localhost:8000/convert/multimodal-to-3d",
					{
						method: "POST",
						body: formData,
					}
	...
	const handleGenerateTextToImage = useCallback(
		async (prompt: string, negativePrompt?: string) => {
			...
			try {
				...
				const response = await fetch(
					"http://localhost:8000/convert/text-to-image",
					{
						method: "POST",
						body: formData,
					}
	...
	const pollTextToImageCompletion = useCallback(
		(taskId: string, promptName: string) => {
				...
				try {
					...
					if (taskStatus.status === "success") {
						...
						toast({
							...
							description:
								"Your text-to-image generation completed successfully.",
						});
					...
					console.error(
						"Error polling text-to-image task status:",
						error
					);
				}
			}, 3000);
				...
				try {
					...
					if (taskStatus.status === "success") {
						...
						const downloadResponse = await fetch(
							`http://localhost:8000/task/${taskId}/download?format=glb`
						);
	...
	const handleDownloadWholeScene = async () => {
		...
		toast({
			...
			description:
				"Creating combined GLB file with lights and advanced editing data, this may take a moment...",
		});
		...
		try {
			...
			const loadPromises = models.map(async (model) => {
				try {
					...
					console.error(
						`Error processing model ${model.name}:`,
						error
					);
				}
			});
			...
			const ambientLight = new THREE.AmbientLight(
				currentLightingFilter === "warm"
					? "#ff9966"
					: currentLightingFilter === "cool"
					? "#6699ff"
					: "#ffffff",
				currentLightingFilter === "soft"
					? 0.8
					: currentLightingFilter === "dramatic"
					? 0.05
					: currentLightingFilter === "warm"
					? 0.4
					: currentLightingFilter === "cool"
			...
			exporter.parse(
				...
				(gltf) => {
					...
					toast({
						...
						description:
							"Complete scene with lighting effects and advanced tools metadata downloaded as GLB file.",
					});
				},
				(error) => {
					...
					toast({
						title: "Export Failed",
						description:
							"Could not export scene. Please try again.",
						variant: "destructive",
					});
				},
				{ binary: true }
	...
	const checkPerformanceBeforeAdding = (
		...
		if (
			impact.wouldExceedLimits ||
			impact.projectedWarningLevel === "critical"
		) {
			setPerformanceDialogData({
				warningLevel: impact.projectedWarningLevel,
				warnings: impact.warnings,
				suggestions: [
					"Remove some models to free up performance",
					"Try using simpler models",
				],
	...
	const handleRemoveLargestModel = () => {
		...
		toast({
			...
			description: `Removed ${largestModel.name} (${(
				largestModel.faces || 0
			).toLocaleString()} faces) to improve performance.`,
		});
	};
	...
	const handleSculptingToolSelect = (brushConfig: any) => {
		...
		toast({
			...
			description: `${
				brushConfig.type.charAt(0).toUpperCase() +
				brushConfig.type.slice(1)
			} brush active - Size: ${brushConfig.size.toFixed(
				1
			)}, Intensity: ${brushConfig.intensity.toFixed(1)}`,
		});
	};
	...
	const handleToolSelect = (tool: string, options?: any) => {
		...
		if (tool === "sculpting-mode") {
			...
			toast({
				title: options?.enabled
					? "Sculpting Mode Enabled"
					: "Sculpting Mode Disabled",
				description: options?.enabled
					? "Advanced SculptGL-inspired vertex manipulation tools are now active"
					: "Returned to standard editing mode",
			});
			return;
		}
		...
		if (tool.startsWith("sculpt-")) {
			...
			toast({
				...
				description: `Selected ${brushType} brush - Size: ${(
					options?.size || 1.0
				).toFixed(1)}, Intensity: ${(options?.intensity || 0.5).toFixed(
					1
				)}`,
			});
			return;
		...
		switch (tool) {
			case "brush":
				toast({
					...
					description: `Texture: ${
						options?.texture
					}, Size: ${options?.size?.toFixed(
						1
					)}, Intensity: ${options?.intensity?.toFixed(1)}`,
				});
				break;
			case "scissors":
				toast({
					...
					description:
						"Click and drag across an object to cut it into separate parts.",
				});
				break;
			case "light-create":
				toast({
					...
					description:
						"Click in the scene to place a new light source.",
				});
				break;
			case "smudge":
				toast({
					...
					description:
						"Click and drag on model surfaces to smudge and deform them.",
				});
				break;
			case "deform":
				toast({
					...
					description:
						"Click on a model, then click and drag in empty space to deform it.",
				});
				break;
			default:
				if (tool.startsWith("lighting-filter")) {
					...
					toast({
						...
						description:
							"Scene lighting has been updated with new filter.",
					});
				}
		}
	};
	...
	const handleSceneClick = (event: any) => {
		if (activeTool === "light-create" && event.point) {
			...
			toast({
				title: "Light Created!",
				description: `New light placed at (${event.point.x.toFixed(
					1
				)}, ${event.point.y.toFixed(1)}, ${event.point.z.toFixed(1)})`,
			});
		}
	};
					...
					<Suspense fallback={null}>
						{models.map((model) => {
							const { key, ...restProps } = {
								...
								onSizeChange: (
									newSize: [number, number, number]
								) => handleModelSizeChange(model.id, newSize),
								keyboardMove:
									...
									switch (toolType) {
										...
										case "deform-start":
											...
											toast({
												...
												description:
													"Click and drag in empty space to deform the selected object.",
											});
											break;
										...
										case "sculpt-grab":
											toast({
												...
												description: `Applied ${toolType.replace(
													"sculpt-",
													""
												)} sculpting to ${model.name}`,
											});
											break;
									}
								},
						...
						{currentView === "generation" ? (
							<>
								<GenerationPanel
									...
									onGenerateMultimodal={
										handleGenerateMultimodal
									}
									onGenerateTextToImage={
										handleGenerateTextToImage
									}
												...
												>
													<div className="flex-1">
														...
														<p className="text-gray-400 text-xs flex items-center gap-1">
															...
															{model.status} •
															Position: (
															{model.position[0].toFixed(
																1
															)}
															,{" "}
															{model.position[2].toFixed(
																1
															)}
															)
														</p>
							...
							<Editing3DSidebar
								...
								onSculptingToolSelect={
									handleSculptingToolSelect
								}
						...
						{currentView === "generation" ? (
							...
							<div className="[#080c0c]/30 backdrop-blur-sm rounded-2xl border border-gray-800/50 p-4">
								<div className="flex items-center justify-between mb-4">
									<h3 className="text-white font-medium flex items-center gap-2">
										<Layers className="w-5 h-5 text-[#c3b383]" />
										Shared Mockups Gallery
									</h3>
									<Button
										variant="ghost"
										size="sm"
								...
								<div className="[#080c0c]/30 backdrop-blur-sm rounded-2xl border border-gray-800/50 p-4">
									...
									<div className="space-y-2 text-sm text-gray-300">
										...
										currentSculptBrush ? (
											<div className="bg-purple-900/30 p-2 rounded-lg border border-purple-500/30">
												<div className="text-purple-200 capitalize font-medium">
													Sculpting:{" "}
													{currentSculptBrush.type}
												</div>
												<div className="text-xs text-purple-300 mt-1">
													Size:{" "}
													{currentSculptBrush.size?.toFixed(
														1
													)}{" "}
													| Intensity:{" "}
													{currentSculptBrush.intensity?.toFixed(
														1
													)}
													{currentSculptBrush.symmetry
														?.enabled &&
														` | Symmetry: ${currentSculptBrush.symmetry.axis.toUpperCase()}`}
												</div>
											</div>
										) : activeTool ? (
											<div className="bg-purple-900/30 p-2 rounded-lg border border-purple-500/30">
										...
										<div className="mt-3 space-y-1">
											...
											<div>
												• Select a model from the scene
											</div>
											<div>
												• Choose tools from the left
												sidebar
											</div>
											{sculptingEnabled ? (
												<>
												...
												<>
													...
													<div>
														...
														Apply instantly to scene
													</div>
													<div>
														•{" "}
														<strong>Deform:</strong>{" "}
														Click model, then drag
														in empty space to deform
													</div>
													<div>
														•{" "}
														<strong>Smudge:</strong>{" "}
														Click models for random
														effects
													</div>
												</>
											)}
													...
													>
														<div className="flex-1">
															...
															<p className="text-gray-400 text-xs">
																Pos: (
																{light.position[0].toFixed(
																	1
																)}
																,{" "}
																{light.position[1].toFixed(
																	1
																)}
																,{" "}
																{light.position[2].toFixed(
																	1
																)}
																)
															</p>
														...
														<Button
															...
															onClick={() => {
																...
																toast({
																	title: "Light Removed",
																	description:
																		"Light source has been deleted from the scene.",
																});
															}}
							...
							<>
								...
								{selectedModel && (
									<div className="mt-4 [#080c0c]/30 backdrop-blur-sm rounded-2xl border border-gray-800/50 p-4">
										...
										<div className="space-y-2 text-sm text-gray-300">
											...
											<div className="flex items-center gap-2">
												<kbd className="px-2 py-1 text-xs [#080c0c] rounded">
													↑
												</kbd>
								...
								<ResizeToolbar
									...
									onResetSize={() =>
										selectedModel &&
										handleResetSize(selectedModel.id)
									}
								/>
